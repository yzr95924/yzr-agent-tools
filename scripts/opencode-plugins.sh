#!/usr/bin/env bash
# Manage opencode-plugins: install | uninstall
#
# Self-contained: writes the bin/opencode-plugins wrapper, links bash/zsh/fish
...
#
# NOTE: these two subcommands manage the *tool* (CLI wrapper on PATH). The
# tool's own subcommands (list/install/uninstall/sync/verify) manage the
# bundled OpenCode plugins — different "install", same words by design.
#
# Usage:
#     scripts/opencode-plugins.sh install      # wrapper + completions + PATH block
#     scripts/opencode-plugins.sh uninstall    # remove all of the above
set -euo pipefail

TOOL="opencode-plugins"
...
Afterwards run '$TOOL install' to also install the bundled OpenCode plugins.
