...
配置工具集合。每个工具都是一个独立的 CLI,通过直接改写 agent 的全局配置实现——无 daemon、
无代理、无协议转换,用户改了之后重启 agent 即生效。

## 本仓库目前包含的工具

| 工具             | 简介                                                          | 状态     |
| ---------------- | ------------------------------------------------------------- | -------- |
| `model-switch`   | 切换 AI coding agent 使用的 Anthropic 兼容模型                | 已发布   |

> 新工具按需添加;同一份仓库规约(测试隔离、原子写、未知字段透传)对所有工具生效。

下面是 `model-switch` 的完整文档:
